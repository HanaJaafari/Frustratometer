...
%
% 	M. Ekeberg, C. L??vkvist, Y. Lan, M. Weigt, E. Aurell, Improved contact
...
function [h,J]=plmDCA_asymmetric(protein,reweighting_threshold,nr_of_cores)
...
%Read inputfile (removing inserts), remove duplicate sequences, and calculate weights and B_eff.
    [N,B_with_id_seq,q,Y]=return_alignment(sprintf('dcaf_%s_alignment_gaps_filtered.fasta',protein));
    Y=unique(Y,'rows');
    [B,N]=size(Y);
    weights = ones(B,1);
...
%Take J_ij as the average of the estimates from g_i and g_j.
    J=0.5*(J1+J2);
%Shift the field estimates into the Ising gauge.
    h=zeros(q,N);
    for r=1:N
        htemp_r=w(1:q,r);
        Jtemp_r=reshape(w(q+1:end,r),q,q,N-1);
        for s=1:q
            h(s,r)=htemp_r(s);
            for i=1:N-1
                h(s,r)=h(s,r)+mean(Jtemp_r(s,:,i));
            end
        end
    end
    for r=1:N
    ...
    save(sprintf('%s_plmdca_parameters.matlab', protein),'h','J')
end
